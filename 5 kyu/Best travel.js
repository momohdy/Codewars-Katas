...
function chooseBestSum(t, k, ls) {
  let combinations = generateCombinations(ls);

  let sumOfCombination = combinations.map((innerArr) =>
    innerArr.reduce((a, b) => a + b)
  );

  sumOfCombination.sort((a, b) => a - b);

  let target;
  sumOfCombination.forEach((innerArr, i) => {
    if (innerArr > t && sumOfCombination[i - 1] <= t) {
      target = sumOfCombination[i - 1];
      return; 
    }
  });
